...
using UnityEngine; using T = UnityEngine.Transform;
...
public class Actor : Activ.Kabuki.XTask{
    ...
    void Start(){
        if (loco == null) loco = new KinematicLocomotion(transform, this);
    }
    ...
    public status Ingest(Transform that, bool consume = true)
        => ε(consume ? Hold(that, allowNull: true, hand: "L")
                            && this["Eat"] % ( Wait(1.2f) && Delete(that) )
                    : this["Eat"]);
    ...
    public status Move(Vector3 u, float scalar, float refDist=10f)
        => loco.Move(u, speed * scalar, refDist);
    ...
    public status Reach(Transform that, float dist = 1f){
        return loco.MoveTowards(that, dist, speed );
    }

    public status Reach(Vector3? that, float scalar=1f) => loco.MoveTo(that .Value, speed * scalar);

    public status Strike(Transform that, float dist=1f, bool message=false) => (that != null)
        ? Seq() % @do?[ Reach(that, dist) ]
              % @do?[ this["Strike"] && (!message || Message("OnStrike", that)) ]
    ...
    action Hold(Transform that, bool allowNull = false, string hand = "R"){
        return Hold(that, hand == "R" ? rightHold : leftHold, allowNull);
    }
